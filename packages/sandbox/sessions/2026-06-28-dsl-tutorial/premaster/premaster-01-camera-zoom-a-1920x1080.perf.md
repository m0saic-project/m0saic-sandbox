...
**13.9s** total  ·  **59** ffmpeg commands

`<sandbox>/sessions/2026-06-28-dsl-tutorial/premaster/./premaster-01-camera-zoom-a-1920x1080.mp4`

## ⤷ Bottleneck

`root:3` — **canvas** · `r/gcolc93/gcolc3/fc13` · 7-deep overlay — 5.4s, **39.1%** of the render. Its own composite is the cost (self 5.3s of 5.4s subtree) — optimize the filtergraph that builds this node, not its children.

Hot path: `root` 100% → `root:3` (canvas) 39.1%

## Heaviest mosaic nodes

| node | what | self | % of render | subtree | cmds |
| --- | --- | ---: | ---: | ---: | ---: |
| `root:3` | canvas | 5.3s | 38.3% | 5.4s | 1 |
| `root` | — | 2.7s | 19.8% | 13.9s | 1 |
| `root:4` | inspector | 2.1s | 15.4% | 3.2s | 1 |
| `root:5` | chrome-status | 0.5s | 3.8% | 1.0s | 1 |
| `root:0` | chrome-header | 0.5s | 3.4% | 0.7s | 1 |
| `root:1` | dsl-string | 0.4s | 2.6% | 0.6s | 1 |
| `root:2:0_text` | — | 0.1s | 1% | 0.1s | 1 |
| `root:0:5_text` | — | 0.1s | 0.9% | 0.1s | 1 |

## Heaviest rectangles (self)

| stableKey | what | self | subtree | overlay |
| --- | --- | ---: | ---: | ---: |
| `r/gcolc93/gcolc3/fc13` | canvas | 5.3s | 5.4s | 7-deep |
| `r` | — | 2.7s | 13.9s | 7-deep |
...
| work | count | total | % |
| --- | ---: | ---: | ---: |
| m0saic render (mosaic node) | 7 | 11.7s | 84.1% |
| Render text source "…" to image | 42 | 1.5s | 10.5% |
| Render text source "…" to video | 10 | 0.8s | 5.4% |
...
| 40 | `root:4` | inspector | 2.1s | m0saic render (mosaic node) |
...
```
root  13.9s · 100% (self 2.7s)
├─ root:3 canvas  5.4s · 39.1% (self 5.3s)  ◄ bottleneck
│  └─ + 1 more leaf  0.1s · 0.8%
├─ root:4 inspector  3.2s · 22.9% (self 2.1s)
│  └─ + 28 more leaves  1.0s · 7.5%
├─ root:5 chrome-status  1.0s · 7.1% (self 0.5s)
│  └─ + 17 more leaves  0.5s · 3.3%
├─ root:0 chrome-header  0.7s · 5.1% (self 0.5s)
│  └─ + 3 more leaves  0.2s · 1.7%
├─ root:1 dsl-string  0.6s · 4.2% (self 0.4s)
│  └─ + 2 more leaves  0.2s · 1.6%
└─ root:2 dsl-narration  0.2s · 1.8% (self 0.1s)
   └─ + 1 more leaf  0.1s · 1%
```
